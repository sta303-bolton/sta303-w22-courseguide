...
- `var_patch`, variance in average yield explained by patch-to-patch variance, after accounting for fixed effects and other sources.

- `var_int`, residual variance after fitting the version of this linear model with the most parameters
...
- Create one final plot that creates a 3x1 figure with Q-Q plots for each simulation. You must determine what the distribution of a _p_ value should be, WHEN THE NULL HYPOTHESIS IS TRUE, and compare it it to the distributions of _p_ values we have generated.
