...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.07}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.07}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.12}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.54}}@{}}
\toprule
...
\addlinespace
\cellcolor{gray!6}{Commenting of code} & \cellcolor{gray!6}{Commenting of code} & \cellcolor{gray!6}{No relevant comments or some comments, but limited or inappropriate.} & \cellcolor{gray!6}{Some reasonable comments, though they would be only limited use to a reader.} & \cellcolor{gray!6}{Reasonable comments, though may be unhelpful to a reader in some sections.} & \cellcolor{gray!6}{Code is well-commented and so that a new reader could pick up and get a strong idea of what was being done, where—even if they are not an R coder.} & \cellcolor{gray!6}{1}\\
...
\cellcolor{gray!6}{Writing sample} & \cellcolor{gray!6}{Conclusion} & \cellcolor{gray!6}{No response OR there is no concluding sentence(s).} & \cellcolor{gray!6}{The conclusion is weak not well supported.} & \cellcolor{gray!6}{A conclusion is present but does not completely summarise the central ideas.} & \cellcolor{gray!6}{A clear conclusion summarizes the central ideas.} & \cellcolor{gray!6}{5}\\
...
\emph{Portfolio assessments aim to help you demonstrate your technical coding, statistical thinking, communication and reflection skills. This portfolio also aims to recap and refresh knowledge from your previous statistics courses as well as building your ability to create quality data visualizations.}

\hypertarget{general-instructions-1}{%
\section{General instructions}\label{general-instructions-1}}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/stawberrypatch-1.pdf}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/echo-FALSE-1.pdf}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/unnamed-chunk-24-1.pdf}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/unnamed-chunk-25-1.pdf}
...
Summarize and paraphrase section \href{https://bookdown.org/roback/bookdown-BeyondMLR/ch-logreg.html\#model-discussion-and-summary}{\textbf{6.7.5 Model Discussion and Summary} (Links to an external site.)} of (Roback \& Legler, 2021a). The first paragraph is key results, and the next 3 paragraphs are limitations. Paraphrase and summarize key results and limitations into two paragraphs, and then write your own conclusion in a few sentences.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.75}}@{}}
\toprule
...
Submission instructions & Submission: Via Quercus surveys
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.10}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.27}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.41}}
...
\hypertarget{rubric-8}{%
\subsubsection{Rubric}\label{rubric-8}}
...
\includegraphics{STA303-Course-guide_files/figure-latex/tom_plot-solution-1.pdf}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/unnamed-chunk-86-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{result\_tom1 }\OtherTok{\textless{}{-}} \FunctionTok{anova}\NormalTok{(}\FunctionTok{lm}\NormalTok{(weight }\SpecialCharTok{\textasciitilde{}} \FunctionTok{as\_factor}\NormalTok{(plant) }\SpecialCharTok{+}\NormalTok{ fertilizer, }\AttributeTok{data =}\NormalTok{ tomatoes),  }\FunctionTok{lm}\NormalTok{(weight }\SpecialCharTok{\textasciitilde{}}\NormalTok{ fertilizer, }\AttributeTok{data =}\NormalTok{ tomatoes))}

\NormalTok{pval }\OtherTok{\textless{}{-}} \FunctionTok{sprintf}\NormalTok{(}\StringTok{"\%.2e"}\NormalTok{, result\_tom1}\SpecialCharTok{$}\StringTok{\textasciigrave{}}\AttributeTok{Pr(\textgreater{}F)}\StringTok{\textasciigrave{}}\NormalTok{[}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Get the RSE and square it}
\NormalTok{overall\_var }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(tom\_final)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{overall\_var}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# Get the RSE and square it}
\NormalTok{error\_var }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(tom1)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{error\_var}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{STA303-Course-guide_files/figure-latex/bee_plot-solution-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{interaction.plot}\NormalTok{(honey}\SpecialCharTok{$}\NormalTok{song, honey}\SpecialCharTok{$}\NormalTok{hive, honey}\SpecialCharTok{$}\NormalTok{honey\_g)}
\end{Highlighting}
\end{Shaded}

\includegraphics{STA303-Course-guide_files/figure-latex/unnamed-chunk-94-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rss\_int }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{((honey}\SpecialCharTok{$}\NormalTok{honey\_g }\SpecialCharTok{{-}} \FunctionTok{predict}\NormalTok{(bee\_int\_mod))}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Degrees of freedom for the model}
\NormalTok{df\_int }\OtherTok{\textless{}{-}}\NormalTok{ n\_int }\SpecialCharTok{{-}} \FunctionTok{nrow}\NormalTok{(}\FunctionTok{summary}\NormalTok{(bee\_int\_mod)}\SpecialCharTok{$}\NormalTok{coefficients)}
\NormalTok{df\_int}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# We can pull the value directly from the model summary with $sigma}
\FunctionTok{summary}\NormalTok{(bee\_int\_mod)}\SpecialCharTok{$}\NormalTok{sigma}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{var\_int }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_int\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{var\_int}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{var\_ab }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_agg\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{{-}}\NormalTok{ var\_int }\SpecialCharTok{/} \DecValTok{3}
\NormalTok{var\_ab}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Notice that the RSE here is just the SD of the average honey production hive{-}to{-}hive}
\FunctionTok{sqrt}\NormalTok{(}\FunctionTok{var}\NormalTok{(honey\_agg\_hive}\SpecialCharTok{$}\NormalTok{honey\_avg\_hive))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# hive to hive variation, not explained by song or song/hive interaction}
\NormalTok{var\_hive }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_hive\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{{-}}\NormalTok{ (}\FunctionTok{summary}\NormalTok{(bee\_agg\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\DecValTok{3}
\NormalTok{var\_hive}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# variance in honey production explained by the interaction of song and hive}
\NormalTok{var\_ab }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_agg\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{{-}}\NormalTok{ var\_int }\SpecialCharTok{/} \DecValTok{3}
\NormalTok{var\_ab}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# variance in honey production explained by hive to hive variance}
\NormalTok{var\_hive }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_hive\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{{-}}\NormalTok{ (}\FunctionTok{summary}\NormalTok{(bee\_agg\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\DecValTok{3}
\NormalTok{var\_hive}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# residual variance (unexplained)}
\NormalTok{var\_int }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(bee\_int\_mod)}\SpecialCharTok{$}\NormalTok{sigma}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{var\_int}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.5\linewidth]{images/m3/boss} \includegraphics[width=0.5\linewidth]{images/m3/friend}

Photos from unsplash.com

\hypertarget{read-in-the-data-and-explore-it}{%
\subsection{Read in the data and explore it}\label{read-in-the-data-and-explore-it}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hosp\_table }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(ontario}\SpecialCharTok{$}\NormalTok{Age, ontario}\SpecialCharTok{$}\NormalTok{Status)}
\NormalTok{hosp\_table}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{STA303-Course-guide_files/figure-latex/logit-1.pdf}
...
\CommentTok{\# plot(shuttle$temperature, shuttle$r/shuttle$m)}

\CommentTok{\# ggplot}
\NormalTok{shuttle }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ temperature, }\AttributeTok{y =}\NormalTok{ r}\SpecialCharTok{/}\NormalTok{m)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{4}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\includegraphics{STA303-Course-guide_files/figure-latex/shuttlePlot-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{shuttle}\SpecialCharTok{$}\NormalTok{notDamaged }\OtherTok{\textless{}{-}}\NormalTok{ shuttle}\SpecialCharTok{$}\NormalTok{m }\SpecialCharTok{{-}}\NormalTok{ shuttle}\SpecialCharTok{$}\NormalTok{r}
\NormalTok{shuttle}\SpecialCharTok{$}\NormalTok{y }\OtherTok{\textless{}{-}} \FunctionTok{as.matrix}\NormalTok{(shuttle[,}\FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}r\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}notDamaged\textquotesingle{}}\NormalTok{)])}
\NormalTok{shuttleFit }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ temperature }\SpecialCharTok{+}\NormalTok{ pressure, }
  \AttributeTok{family=}\FunctionTok{binomial}\NormalTok{(}\AttributeTok{link=}\StringTok{\textquotesingle{}logit\textquotesingle{}}\NormalTok{), }\AttributeTok{data=}\NormalTok{shuttle)}
\NormalTok{shuttleFit}\SpecialCharTok{$}\NormalTok{coef}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{quantile}\NormalTok{(shuttle}\SpecialCharTok{$}\NormalTok{temperature)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{quantile}\NormalTok{(shuttle}\SpecialCharTok{$}\NormalTok{pressure)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{shuttle}\SpecialCharTok{$}\NormalTok{temperatureC }\OtherTok{\textless{}{-}}\NormalTok{ shuttle}\SpecialCharTok{$}\NormalTok{temperature  }\SpecialCharTok{{-}} \DecValTok{70}
\NormalTok{shuttle}\SpecialCharTok{$}\NormalTok{pressureC }\OtherTok{\textless{}{-}}\NormalTok{  shuttle}\SpecialCharTok{$}\NormalTok{pressure }\SpecialCharTok{{-}} \DecValTok{200}
\NormalTok{shuttleFit2 }\OtherTok{\textless{}{-}}  \FunctionTok{glm}\NormalTok{(y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ temperatureC }\SpecialCharTok{+}\NormalTok{ pressureC, }\AttributeTok{family=}\StringTok{\textquotesingle{}binomial\textquotesingle{}}\NormalTok{, }\AttributeTok{data=}\NormalTok{shuttle)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{par\_table }\OtherTok{=} \FunctionTok{cbind}\NormalTok{(}\AttributeTok{est =} \FunctionTok{summary}\NormalTok{(}
\NormalTok{  shuttleFit2)}\SpecialCharTok{$}\NormalTok{coef[,}\DecValTok{1}\NormalTok{],}
  \FunctionTok{confint}\NormalTok{(shuttleFit2))}
\FunctionTok{rownames}\NormalTok{(par\_table)[}\DecValTok{1}\NormalTok{]}\OtherTok{=} \StringTok{"Baseline"}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:shuttleSummaryExp}MLEs of baseline odds and odds ratios, with 95% confidence
intervals.}
\centering
...
\FunctionTok{length}\NormalTok{(}\FunctionTok{unique}\NormalTok{(bacteria}\SpecialCharTok{$}\NormalTok{ID))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# How many weeks?}
\FunctionTok{table}\NormalTok{(bacteria}\SpecialCharTok{$}\NormalTok{week)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Were all children tested each week?}
\FunctionTok{table}\NormalTok{(bacteria}\SpecialCharTok{$}\NormalTok{week)}\SpecialCharTok{/}\DecValTok{50}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bacteria}\SpecialCharTok{$}\NormalTok{newy }\OtherTok{=} \FunctionTok{as.integer}\NormalTok{(bacteria}\SpecialCharTok{$}\NormalTok{y}\SpecialCharTok{==}\StringTok{\textquotesingle{}y\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bacteria}\SpecialCharTok{$}\NormalTok{ap[bacteria}\SpecialCharTok{$}\NormalTok{week }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{=} \StringTok{\textquotesingle{}p\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bacteria}\SpecialCharTok{$}\NormalTok{ap }\OtherTok{=} \FunctionTok{fct\_relevel}\NormalTok{(bacteria}\SpecialCharTok{$}\NormalTok{ap, }\StringTok{"p"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Let\textquotesingle{}s grad the fixed effects}
\FunctionTok{round}\NormalTok{(}\FunctionTok{summary}\NormalTok{(bRes)}\SpecialCharTok{$}\NormalTok{coef, }\AttributeTok{digits=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{STA303-Course-guide_files/figure-latex/unnamed-chunk-164-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Get the estimates transformed}
\NormalTok{ests }\OtherTok{\textless{}{-}} \FunctionTok{format}\NormalTok{(}\FunctionTok{round}\NormalTok{(}\FunctionTok{exp}\NormalTok{(}\FunctionTok{summary}\NormalTok{(bRes)}\SpecialCharTok{$}\NormalTok{coeff)[,}\DecValTok{1}\NormalTok{], }\DecValTok{2}\NormalTok{), }\AttributeTok{nsmall =} \DecValTok{2}\NormalTok{)}
\NormalTok{ests}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{STA303-Course-guide_files/figure-latex/misspecify-5-1.pdf}
...
\vtop{\hbox{\strut Piazza:}\hbox{\strut \url{https://piazza.com/class/kx47tj4fmy65dg?cid=238}}} \\
...
\item
  ...
  \begin{itemize}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=124}{Caption vs figure caption?~(Links to an external site.)}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=119}{Which geom to pick for task 2?~(Links to an external site.)}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=117}{Why can't R find my data?~(Links to an external site.)}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=70_f1}{Wait, shouldn't there be 6 columns?~(Links to an external site.)} (and added to the \href{https://sta303-bolton.github.io/sta303-w22-courseguide/faqs-and-errata.html\#errata}{Errata~(Links to an external site.)})
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=106}{Population mean or simulation mean?~(Links to an external site.)}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=81}{What is the 2 in N(10,2)?}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg?cid=72}{How to get the histograms on top of each other?~(Links to an external site.)}
  \item
    \href{https://piazza.com/class/kx47tj4fmy65dg}{\ldots and so much more!~(Links to an external site.)}
  \end{itemize}
\item
  Made a plan to come to \href{https://q.utoronto.ca/courses/253305/pages/office-hours}{\textbf{office hours}} today/tomorrow/Thursday?
\item
  \href{https://forms.office.com/Pages/ResponsePage.aspx?id=JsKqeAMvTUuQN7RtVsVSEOKHUU3SzAJJhmOKjJhDWEpUNTFDSzhZTFlXUzVYMVlNM1FEUTRZMkVWOC4u}{Remembered the course policy around \textbf{extension requests} and \textbf{declarations of illness/emergency}?~(Links to an external site.)}
\item
  Remembered that you don't actually have to do this assessment if you want to take \href{https://sta303-bolton.github.io/sta303-w22-courseguide/assessmentsoverview.html}{Assessment Path B~(Links to an external site.)}?
\item
  Seen that you could start on the \href{https://q.utoronto.ca/courses/253305/discussion_topics/1603374}{main portfolio writing sample already?}
\end{itemize}
\end{minipage} \\
\href{https://q.utoronto.ca/courses/253305/discussion_topics/1608011}{Help notes pre mini-portfolio submission} & 2022-02-02 2:38 p.m. ET & My \href{https://q.utoronto.ca/courses/253305/pages/office-hours}{\textbf{last office hour before the submission of the mini-portfolio is today, 4:10 to 5:00 p.m. ET}} (after the workshop in the first half, same link). All are welcome, regardless of section enrolment or attendance at the first part.
...
\vtop{\hbox{\strut There will also be a \href{https://q.utoronto.ca/courses/253305/pages/office-hours}{\textbf{TA office hour tomorrow from noon to 1:00 p.m.}} (and one at 5, but that is after the due time, which is, of course, 3:03 p.m. ET on \href{https://markus-ds.teach.cs.toronto.edu/courses}{MarkUs}).}\hbox{\strut Please make sure you're \textbf{knitting early and often} and have checked out some of the Piazza posts I highlighted as \textbf{\emph{very helpful}} in \href{https://q.utoronto.ca/courses/253305/discussion_topics/1605315}{this announcement}. Test out your \textbf{access to \href{https://markus-ds.teach.cs.toronto.edu/courses}{MarkUs}} and upload as many times as you like before the deadline.}}
