...
\subsection{Step 1: Decide on roles}\label{step-1-decide-on-roles}}
...
\subsection{Step 2: Driver sets up group}\label{step-2-driver-sets-up-group}}
...
\subsection{Step 3: Members join}\label{step-3-members-join}}
...
\subsection{Step 4: Complete the activity}\label{step-4-complete-the-activity}}
...
\subsection{Step 5: The Driver MUST click Submit to Quercus for your answers to be processed.}\label{step-5-the-driver-must-click-submit-to-quercus-for-your-answers-to-be-processed.}}
...
\emph{How exactly do the tutorials and lectures work?}
...
\emph{``I don't really understand the knowledge basket, how will that work? Can we earn more than 5 marks?''}

The knowledge basket maxes out at 5\%, otherwise I get in trouble for giving away too many ``free'' points. (I know, I wish.)

\emph{``Can we do as many knowledge basket activities as we want in order to gain the full 5\%?''}
...
\emph{``How fast will we receive grades back for the knowledge basket activities?''}
...
\emph{``I am not too sure what `pre-knowledge quiz: 80\% or workshop' is supposed to mean.''}
...
\emph{``What will additional opportunities like speaker series reflections be like?''}
...
\emph{``Are the three phases of each writing assignment graded on completion?''}
...
\emph{``No questions, although seems to be a hectic course with a lot of minor deadlines.''}
...
\emph{``I am curious about the format of portfolio and assessment and how they will be conducted.''}
...
\emph{``How does the mixed assessment work? Is it like a midterm?''}
...
\emph{``What's the difference between mini and mixed assessments? Are they both term tests?''}
...
\emph{Will we get some sample practice problems to work on before the mini-mixed and mixed assessments?}
...
\emph{``What exactly will we do in assessment weeks?''}
...
\emph{``What kind of activities will the final project entail?''}
...
\emph{``How we can choose path A or B for the marking scheme-- would this be on Quercus?''}
...
\emph{``So are all assessment due on Thursday 3:03?''}
...
\emph{``Are we allowed to answer assignments with programming languages other than R?''}

This is an R-based course, so only coding assessments using R will be accepted.

\emph{``What are the minimum requirements to pass the course?''}
...
\emph{My screen seems to be lagging compared to those of my group members (e.g., I'm not on the same question as the driver/members, the answer I chose does not appear on my group members' screens, etc.)}
...
\emph{Can you (Prof.~Bolton) write me a reference?}
